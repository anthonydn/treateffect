...
gg <- ggplot(data = x2) +
  xlab(d$times) +
  ylab(paste(d$response, ", ", cen, " ± ", bars)) +
...
if (x_axis == "time") {
  ...
  if (points) gg <- gg + geom_point(aes_string(d$times, d$response,
...
if (x_axis == "treatment") {
  ...
  if (is.null(panel.eq)&!is.null(d$times))
    panel.eq <- panel.default(c(d$panel,"times"))
  if (bars == "box") gg <- gg +
    geom_boxplot(aes_string(d$treatment,d$response, col = d$treatment),
	data = x$data) else
  gg <- gg + geom_linerange(aes_string(d$treatment, "cen", ymax = "hi",
    ymin = "lo", col = d$treatment), position = pd, lwd = 1)
  if (points) gg <- gg + geom_point(aes_string(d$treatment, d$response,
    col = d$treatment), data = x$data, position = pd)
...
panel.default <- function(x) {
   ...
   if(length(x)>2) peq <- as.formula(paste(x[1],"~",paste0(x[-1],collapse="+")))
